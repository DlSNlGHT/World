...
4. name 必须是字符串数组。数组内只能放对话中明确指向同一人物的姓名、称号或别名；不要猜测别名，不要把被记住的人放进 name。
5. memory 必须明确写出记忆持有者，使用第三人称完整句，不能使用含混的“他/她记得这件事”。每条 memory 最多 50 个字符（包含标点）；超过时必须压缩措辞，若含多条独立记忆则拆成多项，禁止生硬截断或输出残句。
6. time 表示记忆所指事件发生的绝对故事时间，而不是提取时间。只能填写不会随当前时间变化的具体时间，例如“2005年10月1日”“帝国历1024年11月3日 夜间”“第12日 22:30”。
7. 禁止把“昨晚、三天前、刚才、不久前、宴会之后”等相对时间原样写入 time。只有在输入提供了明确的当前故事时间且能够唯一换算时，才转换成绝对时间；否则 time 必须为空字符串。
8. 不要因为已有记忆与新内容冲突而修改、覆盖或删除任何一方；只提取本批对话中新形成或被明确唤起的主观记忆。
9. 每个人物本批最多输出 3 条，整批最多输出 8 条。超过时按“未来影响最大、最可能长期保留、最能改变人物行为”的顺序取舍；宁缺毋滥。
10. 保留原文的认知强度：怀疑仍是怀疑，推测仍是推测，不能改成确信。
11. 没有值得记录的主观记忆时返回空数组 []。
...
数组中每一项严格使用：
{"name":["人物主名称","明确别名"],"memory":"不超过50个字符的一条完整主观记忆。","time":"绝对故事时间或空字符串"}

除 name、memory、time 外不得增加其他字段。`;
