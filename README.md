...
入口是 `world-engine.js`。它按 `MODULES` 数组顺序动态加载所有模块，然后：

1. 调用 `WORLD_ENGINE_STORE.hydrate()`，将 IndexedDB/旧 `localStorage` 数据灌入同步内存镜像。
2. 初始化 `WORLD_ENGINE_CHATCACHE`，恢复当前聊天的实时缓冲和存档。
3. 初始化只读注入检查器。
4. 初始化记忆引擎事件监听。
5. 加载世界引擎规则并绑定世界引擎事件。
6. 创建面板和悬浮球。

模块都是 IIFE，通过 `window.*` 暴露接口，没有打包器或构建步骤。新增模块时必须注意依赖顺序：被依赖模块必须先进入 `MODULES`。
...
1. 在 `world-engine.js` 的 `MODULES` 中按依赖顺序加载新模块。
