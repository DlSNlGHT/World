...
【创建准入：必须先判断，再分类】
叙事中出现过的行动、冲突或重大结果，不自动成为事件链。只有同时满足以下条件，才允许创建非终局事件链：
1. 存在一个具体、可描述的未解决目标或持续矛盾，而不是笼统的“可能还有后续”。
2. 结果尚未确定；仍有至少两种合理走向。
3. 存在有动机且有能力继续行动的主体。
4. 该事项需要跨未来轮次追踪，不能在当前场景内自然处理完毕。

以下内容不得创建为非终局事件链：
- 已经完成、胜负已定、目标已达成或失败、冲突已经实际结束的行动。
- 只有历史意义、新闻价值或情绪余波，但没有待解决目标的事件。
- 一次性对话、普通赶路、日常交易、短暂气氛和当前场景内即可收束的小事。
- 仅凭“可能引发报复、调查、善后”等推测，把尚未实际出现的后续当成进行中事件。

已经闭合的重大事件，应根据实际后果写入世界摘要、风声、声誉、势力、经济或影响链，不得从“萌芽/筹备”重新开始。只有出现了独立、明确且尚未完成的新目标（如战后追剿残部、重建被毁粮道），才能以该新目标另建事件链。
若新建条目所描述的事情在本轮已经走到结果，必须直接使用对应终局阶段，不得为了走流程故意降回早期阶段。
